...
if not ANTHROPIC_API_KEY:
    logger.warning("ANTHROPIC_API_KEY nicht gesetzt. Claude-Integration wird nicht funktionieren.")
...
class MCPRequest(BaseModel):
    ...
    server_name: str = Field(..., description="Name des MCP-Servers (z.B. puppeteer, playwright)")
    action: str = Field(..., description="Aktion, die ausgeführt werden soll (z.B. navigate, screenshot)")
    parameters: Dict[str, Any] = Field(default_factory=dict, description="Parameter für die Aktion")
...
class ClaudeClient:
        ...
        for server_name, config in self.mcp_config.get("mcpServers", {}).items():
            ...
            if "puppeteer" in server_name:
                tools.append(MCPTool(
                    ...
                    server=server_name,
                    parameters={"url": {"type": "string", "description": "URL, zu der navigiert werden soll"}}
                ))
                tools.append(MCPTool(
                    name="puppeteer_screenshot",
                    description="Mache einen Screenshot der aktuellen Seite",
                    server=server_name,
                    parameters={"name": {"type": "string", "description": "Name für den Screenshot"}}
                ))
                ...
                tools.append(MCPTool(
                    ...
                    server=server_name,
                    parameters={"problem": {"type": "string", "description": "Problem, das gelöst werden soll"}}
                ))
        return tools
    ...
    def execute_mcp_request(self, request: MCPRequest) -> MCPResponse:
        ...
        if server_name not in self.mcp_config.get("mcpServers", {}):
            ...
            return MCPResponse(success=False, error=f"MCP-Server '{server_name}' nicht gefunden")
        ...
        if not server_url:
            ...
            return MCPResponse(success=False, error=f"Keine URL für MCP-Server '{server_name}' konfiguriert")
    ...
    def chat(self, message: str) -> str:
        ...
        try:
            ...
            while True:
                ...
                current_pos = json_end + 3  # Nach dem Ende dieses JSON-Blocks fortsetzen
            ...
            if all_mcp_requests:
                    ...
                    self.conversation_history.append(ChatMessage(role="assistant", content=claude_response))
                    ...
                    for i, mcp_request_data in enumerate(all_mcp_requests):
                        ...
                        logger.info(f"MCP-Anfrage {i+1}/{len(all_mcp_requests)}: {action} auf {server_name}")
                    ...
                    updated_messages.append({"role": "assistant", "content": claude_response})
