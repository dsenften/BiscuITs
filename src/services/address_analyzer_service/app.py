...
def main():
    while True:
        try:
            ...
            channel.queue_declare(
                queue="document_processor_input"
            )  # Queue für den Document Processor
            break
    ...
    def callback(ch, method, properties, body):
        try:
            ...
            channel.basic_publish(
                ...
                body=json.dumps(
                    {"name": name, "address": address, "initial_result": initial_result}
                ),
            )
    ...
    channel.basic_consume(
        queue="address_analyzer_input", on_message_callback=callback, auto_ack=True
    )
